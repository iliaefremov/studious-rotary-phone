...
const Schedule: React.FC = () => {
  ...
  const DayCard: React.FC<{ dayData: DaySchedule, weekNumber: 1 | 2, date: Date }> = ({ dayData, weekNumber, date }) => {
    ...
    return (
        <div className={`flex-shrink-0 w-full flex flex-col p-4 rounded-lg shadow-md transition-all duration-300 ${isToday ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-center mb-4">
            <h3 className={`text-lg font-bold ${isToday ? 'text-blue-700' : 'text-gray-800'}`}>{dayName}</h3>
                <span className={`text-sm font-medium px-3 py-1 rounded-full ${isToday ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>{formattedDate}</span>
            </div>
            <div className="space-y-3">
            {classes.length > 0 ? classes.map((item) => {
                const isNow = isToday && isCurrentTimeInRange(item.time);
                return (
                <div key={item.id} className={`group bg-white rounded-lg p-3 flex flex-col text-sm shadow-sm transition-all duration-300 ease-in-out border border-gray-200 ${isNow ? 'ring-2 ring-blue-400 shadow-lg' : 'hover:shadow-md'}`}>
                    <div className="flex-grow">
                        <div className="flex justify-between items-start mb-2"><p className="font-bold text-gray-800 pr-2 leading-tight">{item.subject}</p>{item.isImportant && <span className="text-base" title="Важное">⭐</span>}</div>
                        <div className="space-y-1 text-gray-600 text-xs mb-3"><p className="flex items-center"><span className="opacity-75 mr-2">⏰</span><span className="font-medium">{item.time}</span></p><p className="flex items-start"><span className="opacity-75 mr-2 pt-0.5">📍</span><span>{item.classroom} / {item.teacher}</span></p></div>
                        <div className="text-xs"><p className="text-gray-600 font-medium mb-1">📝 Домашнее задание:</p><p className="text-gray-800 break-words leading-snug">{item.homework}</p></div>
                    </div>
                    {isAdminMode ? (
                        <div className="flex items-center justify-between pt-3 mt-3 border-t border-gray-200">
                            <button onClick={() => handleGetHelpClick(item)} className="bg-blue-500 text-white font-bold py-1.5 px-3 rounded-md text-xs flex items-center justify-center space-x-2 hover:bg-blue-600 transition-colors shadow-sm">✨ AI</button>
                            <div className="flex items-center space-x-2">
                                <button onClick={() => handleOpenEditModal(item, dayName, weekNumber)} className="bg-gray-200 text-gray-700 hover:bg-gray-300 transition-colors p-1.5 rounded-md shadow-sm">✏️</button>
                                <button onClick={() => handleDeleteItem(item.id, dayName, weekNumber)} className="bg-red-500 text-white hover:bg-red-600 transition-colors p-1.5 rounded-md shadow-sm">🗑️</button>
                            </div>
                        </div>
                    ) : (
                        <button onClick={() => handleGetHelpClick(item)} className="mt-3 w-full bg-blue-500 text-white font-bold py-2 px-3 rounded-md text-xs flex items-center justify-center space-x-2 hover:bg-blue-600 transition-colors shadow-sm"><span>✨</span><span>AI Помощник</span></button>
                    )}
                </div>
            );
            }) : (
                <div className={`rounded-lg text-center text-gray-600 flex items-center justify-center border-2 border-dashed border-gray-300 p-4 min-h-[100px]`}>
                <p className="text-sm">Пар нет, можно отдохнуть!</p>
                </div>
            )}
            {isAdminMode && (<button onClick={() => handleOpenEditModal(null, dayName, weekNumber)} className="w-full bg-green-500 text-white font-bold py-2 px-3 rounded-lg text-sm hover:bg-green-600 transition-colors mt-3 shadow-sm">+ Добавить пару</button>)}
            </div>
        </div>
    );
  };
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-2 mt-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Сегодня: {todayDateFormatted}</p>
        </div>
      </div>
       {/* Кнопки AI-инструментов */}
       <div className="flex flex-wrap items-center justify-start gap-2 mb-6">
          <button onClick={openSummaryModal} className="bg-blue-500 text-white font-bold py-2 px-4 rounded-lg text-sm flex items-center justify-center space-x-2 hover:bg-blue-600 transition-colors shadow-md"><span>📊</span><span>Сводка недели</span></button>
          <button onClick={openPlanModal} className="bg-blue-500 text-white font-bold py-2 px-4 rounded-lg text-sm flex items-center justify-center space-x-2 hover:bg-blue-600 transition-colors shadow-md"><span>📝</span><span>План на неделю</span></button>
        </div>
      ...
      <div className="flex space-x-2 mb-6">
        <button
          ...
          className={`w-1/2 p-2 rounded-lg text-sm font-bold transition-all duration-300 ${activeWeek === 1 ? 'bg-blue-500 text-white shadow-md' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
        >
          Первая неделя {currentCalendarWeek === 1 && <span className="text-xs">(текущая)</span>}
        </button>
        <button
          onClick={() => setActiveWeek(2)}
          className={`w-1/2 p-2 rounded-lg text-sm font-bold transition-all duration-300 ${activeWeek === 2 ? 'bg-blue-500 text-white shadow-md' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
        >
      ...
      {isHelpModalOpen && modalContent && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 animate-fade-in"><div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-zoom-in border border-gray-200"><div className="flex justify-between items-center mb-4"><h3 className="text-2xl font-bold text-blue-700">✨ AI Помощник</h3><button onClick={closeHelpModal} className="text-gray-500 hover:text-blue-700 text-2xl">❌</button></div><div className="mb-4"><p className="font-semibold text-gray-600">Ваш вопрос по предмету "{modalContent.subject}":</p><p className="p-3 bg-gray-100 rounded-lg mt-2 text-gray-800">{modalContent.homework}</p></div><button onClick={fetchAiHelp} disabled={isHelpLoading} className="w-full bg-blue-500 text-white font-bold py-2.5 px-4 rounded-lg flex items-center justify-center space-x-2 hover:bg-blue-600 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed shadow-md">{isHelpLoading ? (<><svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><span>Думаю...</span></>) : (<><span className="text-lg">💡</span><span>Получить объяснение</span></>)}</button>{helpError && <p className="text-red-500 mt-4 text-center">{helpError}</p>}{aiHelpResponse && <div className="mt-6 p-4 bg-gray-100 rounded-lg"><h4 className="text-lg font-semibold text-blue-700 mb-2">Ответ AI:</h4><div className="text-gray-800 whitespace-pre-wrap">{aiHelpResponse}</div></div>}</div></div>
      )}
       {isPlanModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 animate-fade-in"><div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto animate-zoom-in border border-gray-200"><div className="flex justify-between items-center mb-4"><h3 className="text-2xl font-bold text-blue-700">📝 AI Планировщик Учебы</h3><button onClick={closePlanModal} className="text-gray-500 hover:text-blue-700 text-2xl">❌</button></div><div className="mb-4"><p className="font-semibold text-gray-600">AI создаст план на основе важных задач на выбранной неделе:</p><ul className="list-disc pl-5 p-3 bg-gray-100 rounded-lg mt-2 text-gray-800">{activeWeekScheduleData.flatMap(d => d.classes).filter(i => i.isImportant).length > 0 ? (activeWeekScheduleData.flatMap(d => d.classes).filter(i => i.isImportant).map((item) => (<li key={item.id}><strong>{item.subject}:</strong> {item.homework}</li>))) : (<li>На этой неделе нет важных заданий. Отличное время для повторения!</li>)}</ul></div><button onClick={fetchStudyPlan} disabled={isPlanLoading} className="w-full bg-blue-500 text-white font-bold py-2.5 px-4 rounded-lg flex items-center justify-center space-x-2 hover:bg-blue-600 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed shadow-md">{isPlanLoading ? (<><span>Составляю план...</span></>) : (<><span className="text-lg">🧠</span><span>Создать учебный план</span></>)}</button>{planError && <p className="text-red-500 mt-4 text-center">{planError}</p>}{aiPlanResponse && <div className="mt-6 p-4 bg-gray-100 rounded-lg"><h4 className="text-lg font-semibold text-blue-700 mb-2">Ваш персональный план:</h4><div className="text-gray-800 whitespace-pre-wrap prose prose-sm max-w-none">{aiPlanResponse}</div></div>}</div></div>
      )}
      {isSummaryModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 animate-fade-in"><div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto animate-zoom-in border border-gray-200"><div className="flex justify-between items-center mb-4"><h3 className="text-2xl font-bold text-blue-700">📊 Сводка на неделю</h3><button onClick={closeSummaryModal} className="text-gray-500 hover:text-blue-700 text-2xl">❌</button></div><p className="mb-4 text-gray-600">AI проанализирует ваше расписание и выделит самое главное для выбранной недели.</p><button onClick={fetchWeekSummary} disabled={isSummaryLoading} className="w-full bg-blue-500 text-white font-bold py-2.5 px-4 rounded-lg flex items-center justify-center space-x-2 hover:bg-blue-600 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed shadow-md">{isSummaryLoading ? (<><span>Генерирую сводку...</span></>) : (<><span className="text-lg">💡</span><span>Получить сводку</span></>)}</button>{summaryError && <p className="text-red-500 mt-4 text-center">{summaryError}</p>}{aiSummaryResponse && <div className="mt-6 p-4 bg-gray-100 rounded-lg"><h4 className="text-lg font-semibold text-blue-700 mb-2">Ключевые моменты недели:</h4><div className="text-gray-800 whitespace-pre-wrap prose prose-sm max-w-none">{aiSummaryResponse}</div></div>}</div></div>
      )}
    </div>
  );
};
...
const ScheduleEditModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 animate-fade-in"><div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto animate-zoom-in border border-gray-200"><div className="flex justify-between items-center mb-4"><h3 className="text-2xl font-bold text-blue-700">{isNew ? 'Добавить пару' : 'Редактировать пару'}</h3><button onClick={onClose} className="text-gray-500 hover:text-blue-700 text-2xl">❌</button></div><form onSubmit={handleSubmit} className="space-y-4"><div><label className="text-sm font-bold text-gray-600">Предмет</label><input type="text" name="subject" value={formData.subject} onChange={handleChange} className="w-full bg-gray-100 border border-gray-300 text-gray-800 p-2.5 rounded-lg mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" required /></div><div><label className="text-sm font-bold text-gray-600">Время (напр. 9:00 - 10:30)</label><input type="text" name="time" value={formData.time} onChange={handleChange} className="w-full bg-gray-100 border border-gray-300 text-gray-800 p-2.5 rounded-lg mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" required /></div><div><label className="text-sm font-bold text-gray-600">Аудитория</label><input type="text" name="classroom" value={formData.classroom} onChange={handleChange} className="w-full bg-gray-100 border border-gray-300 text-gray-800 p-2.5 rounded-lg mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" required /></div><div><label className="text-sm font-bold text-gray-600">Преподаватель</label><input type="text" name="teacher" value={formData.teacher} onChange={handleChange} className="w-full bg-gray-100 border border-gray-300 text-gray-800 p-2.5 rounded-lg mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" required /></div><div><label className="text-sm font-bold text-gray-600">Домашнее задание</label><textarea name="homework" value={formData.homework} onChange={handleChange} rows={3} className="w-full bg-gray-100 border border-gray-300 text-gray-800 p-2.5 rounded-lg mt-1 focus:outline-none focus:ring-2 focus:ring-blue-500" required ></textarea></div><div className="flex items-center"><input type="checkbox" name="isImportant" id="isImportant" checked={formData.isImportant} onChange={handleChange} className="h-4 w-4 rounded border-gray-300 text-blue-500 focus:ring-blue-500" /><label htmlFor="isImportant" className="ml-2 text-sm text-gray-800">Это важное задание</label></div><button type="submit" className="w-full bg-blue-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-600 transition-colors shadow-md">Сохранить</button></form></div></div>
  )
};
